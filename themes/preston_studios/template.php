<?php
function preston_studios_menu_local_task($link, $active = FALSE) {
  return '<li '. ($active ? 'class="active" ' : '') .'><span><span>'. $link ."</span></span></li>\n";
}

function preston_studios_l($text, $path, $options = array()) {
  // Merge in defaults.
  $options += array(
      'attributes' => array(),
      'html' => TRUE,
...
*/
function preston_studios_links($links, $attributes = array('class' => 'links')) {
 $output = '';
  if (count($links) > 0) {
...
// In the line above, you could take out the drupal_attributes var and specify your class
       ...
       $output .= preston_studios_l($spanned_title, $link['href'], $link['attributes'], $link['query'], $link['fragment']);
      } else if ($link['title']) {
...
$directory = 'http://www.prestonstudios.com/sites/prestonstudios/themes/preston_studios';
